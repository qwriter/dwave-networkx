...
def pegasus_layout(G, scale=1., center=None, dim=2, crosses=False):
    ...
    ----------
    G : NetworkX graph
        A Pegasus graph or a :term:`subgraph` of a Pegasus graph, as produced by
...
def pegasus_node_placer_2d(G, scale=1., center=None, dim=2, crosses=False):
    """Generate a function to convert :term:`Pegasus` indices to plottable coordinates.

    Parameters
    ----------
    G : NetworkX graph
        A Pegasus graph or a :term:`subgraph` of a Pegasus :term:`graph` as produced by
        the :func:`dwave_networkx.pegasus_graph` function.
...
def draw_pegasus(G, crosses=False, **kwargs):
    """Draw :term:`graph` ``G`` in a :term:`Pegasus` topology.
...
def draw_pegasus_embedding(G, *args, **kwargs):
    """Draw an embedding onto :term:`Pegasus` :term:`graph` ``G``.
